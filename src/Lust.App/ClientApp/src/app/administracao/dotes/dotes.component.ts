...
import { DotesService } from './dotes.service';
...
@Component({
  selector: 'app-dotes',
  templateUrl: './dotes.component.html',
  styleUrls: ['./dotes.component.scss'],
  animations: fuseAnimations
})

export class DotesComponent implements OnInit {
  ...
  constructor(
    private dotesService: DotesService,
  ...
  ngOnInit() {
    this.dataSource = new FilesDataSource(this.dotesService, this.paginator, this.sort);
  ...
  deleteDote(dote) {
    ...
    this.confirmDialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.dotesService.deleteDote(dote)
          .then(deletado => {
            this.snackBar.open('Dote deletado', 'OK', {
...
export class FilesDataSource extends DataSource<any>
{
  ...
  get totalRows(): number {
    return this.dotesService.totalRows;
  }
  ...
  constructor(
    ...
    merge(...displayDataChanges).subscribe(q => {
      ...
      if (skip !== undefined && take !== undefined) {
        ...
        return this.dotesService.getDotes(skip, take, query, _sort.active, _sort.direction);
      }
    });

    this.dotesService.onTotalRowsChanged.subscribe(q => {
      this._paginator.pageIndex = 0;
    })
  ...
  connect(): Observable<any[]> {
    ...
    return this.dotesService.onDotesChanged.pipe(map(() => {

      return this.dotesService.dotes;
    }
    ));
  }
