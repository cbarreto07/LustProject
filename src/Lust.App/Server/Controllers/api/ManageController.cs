...
using Lust.App.Server.ViewModels.ManageViewModels;
...
namespace Lust.App.Server.Controllers.api
{
    ...
    public class ManageController : BaseController
    {
        ...
        {
            var contato = await _clienteAppService.ObterPorIdAsync(_user.GetUserId());
            return Response(contato);
        }

        [HttpGet("contatos")]
        public async Task<IActionResult> Contatos()
        {
            var contatos  =await _clienteAppService.ContatosDoClienteLogadoAsync();
        ...
        {
            var preferencia = await _clienteAppService.ObterPreferenciasAsync(_user.GetUserId());
            return Response(preferencia);
        }
